...
class App(QWidget):
    ...
    QGroupBox::title {
        ...
        /* Nur bestimmte Überschriften im Code separat fett setzen */
        QCheckBox {
            color: black;
            background: transparent;
            font-size: 11pt;
        ...
        self.setWindowTitle("🚒 Probefahrten FW Lauerz 🚒")
        self.resize(900, 650)
        ...
        # BLOECKE
        # =====================================================
        box_allgemein = QGroupBox("Allgemein")
        box_fahrzeuge = QGroupBox("Fahrzeuge")
        box_poseidon = QGroupBox("Poseidon")
        ...
        box_allgemein.setFont(title_font)
        box_fahrzeuge.setFont(title_font)
        box_poseidon.setFont(title_font)

        lay_allgemein = QVBoxLayout()
        lay_fahrzeuge = QVBoxLayout()
        lay_poseidon = QVBoxLayout()

        box_allgemein.setLayout(lay_allgemein)
        box_fahrzeuge.setLayout(lay_fahrzeuge)
        box_poseidon.setLayout(lay_poseidon)

        # =====================================================
        # JAHR
        # =====================================================
        self.year = QLineEdit(str(datetime.now().year))

        lbl = QLabel("Jahr")
        lbl.setStyleSheet("font-weight: bold;")
        lay_allgemein.addWidget(lbl)
        lay_allgemein.addWidget(self.year)
        ...
        btn1 = QPushButton("Mannschaftsliste wählen")
        def choose_file(self):
            path = QFileDialog.getOpenFileName(
                self,
                "Mannschaftsliste wählen",
                "",
                "Excel (*.xlsx *.xls)"
            )[0]
            ...
            if path:
                ...
                self.load_persons_into_dropdowns()


        btn1.clicked.connect(self.choose_file)

        lbl = QLabel("Mannschaftsliste (Lodur)")
        lbl.setStyleSheet("font-weight: bold;")
        top_grid.addWidget(lbl, 0, 0)
        top_grid.addWidget(self.file, 1, 0)
        top_grid.addWidget(btn1, 2, 0)
        ...
        btn2 = QPushButton("Vorlage wählen")
        btn2.clicked.connect(
            lambda: self.template.setText(
        ...
        lbl = QLabel("Vorlage Probefahrt")
        lbl.setStyleSheet("font-weight: bold;")
        top_grid.addWidget(lbl, 0, 1)
        top_grid.addWidget(self.template, 1, 1)
        top_grid.addWidget(btn2, 2, 1)

        lay_allgemein.addLayout(top_grid)

        # ------------------------------------------------
        # Keine Probefahrt
        # ------------------------------------------------
        self.disable = [False] * 12
        ...
        for i, m in enumerate(MONTHS):
            ...
            cb.stateChanged.connect(
                lambda s, i=i:
                self.set_bool(self.disable, i, s)
            )
            grid.addWidget(cb, i // 4, i % 4)

        lbl = QLabel("Monate ohne Probefahrten (Mannschaftsübungen)")
        lbl.setStyleSheet("font-weight: bold;")
        lay_fahrzeuge.addWidget(lbl)
        lay_fahrzeuge.addLayout(grid)

        # ------------------------------------------------
        # Poseidon
        # ------------------------------------------------
        self.pose_disable = [False] * 12
        self.pose_text = [""] * 12

        pose_grid = QGridLayout()

        # Überschrift
        lbl = QLabel("Monate ohne Probefahrten und Freitext / Monat")
        lbl.setStyleSheet("font-weight: bold;")
        lay_poseidon.addWidget(lbl)
        ...
        for i, m in enumerate(MONTHS):
            ...
            cb.stateChanged.connect(
                lambda s, i=i:
                self.set_bool(self.pose_disable, i, s)
            )
            ...
            # -----------------------------------
            # LINKE SPALTE = Jan bis Jun
            # RECHTE SPALTE = Jul bis Dez
            # -----------------------------------
            if i < 6:
                row = i
                col = 0
            ...
            pose_grid.addWidget(cb, row, col)
            pose_grid.addWidget(txt, row, col + 1)

        lay_poseidon.addLayout(pose_grid)

        # ------------------------------------------------
        # Grossreinigung
        # ------------------------------------------------
        lbl = QLabel("Grossreinigung")
        lbl.setStyleSheet("font-weight: bold;")
        lay_poseidon.addWidget(lbl)

        gross_grid = QGridLayout()

        self.gross = QComboBox()
        self.gross.addItems(MONTHS)

        # -----------------------------------
        # 3 Personen für Grossreinigung
        # -----------------------------------
        self.gross_persons = []

        for i in range(3):
            cb = QComboBox()
            cb.addItem("")  # leer = automatische Planung
            self.gross_persons.append(cb)

        # Freitextfeld schmal halten
        self.gross_date = QLineEdit()

        # Linke Spalte
        gross_grid.addWidget(QLabel("Monat"), 0, 0)
        gross_grid.addWidget(self.gross, 1, 0)

        # Rechte Spalte
        gross_grid.addWidget(QLabel("Datum"), 0, 1)
        gross_grid.addWidget(self.gross_date, 1, 1)

        gross_grid.addWidget(QLabel("Personen"), 2, 0)

        for i, cb in enumerate(self.gross_persons):
            gross_grid.addWidget(cb, 3 + i, 0, 1, 2)

        # Breitenverhältnis 1/3 zu 2/3
        gross_grid.setColumnStretch(0, 1)
        gross_grid.setColumnStretch(1, 2)
        gross_grid.setColumnStretch(2, 0)

        # Button
        btn = QPushButton("Generieren")
        ...
        gross_grid.addWidget(btn, 1, 2)

        # schöne Abstände
        gross_grid.setColumnStretch(0, 1)
        gross_grid.setColumnStretch(1, 1)
        gross_grid.setColumnStretch(2, 0)

        lay_poseidon.addLayout(gross_grid)

        layout.addWidget(box_allgemein)
        layout.addWidget(box_fahrzeuge)
        layout.addWidget(box_poseidon)

        self.setLayout(layout)

    def set_bool(self, arr, i, state):
        arr[i] = bool(state)

    def set_text(self, i, t):
        self.pose_text[i] = t
    ...
    def load_persons_into_dropdowns(self):

        if not self.file.text():
            return

        df = pd.read_excel(self.file.text())
    ...
    def generate(self):
        ...
        out_pdf = run(
            ...
            self.template.text(),
            self.year.text(),
            [i for i, v in enumerate(self.disable) if v],
            {
                "disable": self.pose_disable,
                "text": self.pose_text,
                "gross_m": self.gross.currentIndex(),
                "gross_d": self.gross_date.text(),
                "gross_persons": [cb.currentText() for cb in self.gross_persons]
            }
        )
